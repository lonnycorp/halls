...
pub fn player_rotation_update(
    ...
    state.rotation.x =
        (state.rotation.x - window.mouse_delta().y * sensitivity).clamp(-PITCH_LIMIT, PITCH_LIMIT);
    state.rotation.y = (state.rotation.y - window.mouse_delta().x * sensitivity).rem_euclid(TAU);
}
